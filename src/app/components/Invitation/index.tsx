...
import { InvitationStyled } from './styled';

export default function Invitation({ message }: { message: string }) {
  return (
    <InvitationStyled className={clsx('invitation')}>
      <Sections>
        <IconPost className={clsx('ico-post')} />
        <Text>{message}</Text>
      </Sections>
    </InvitationStyled>
  );
...
function IconPost({ className }: { className?: string }) {
  return (
    <svg className={className} data-name="Livello 1" id="Livello_1" viewBox="0 0 128 128">
      <title />
