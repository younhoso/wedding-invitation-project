...
import { IconHeartSvgStyled, IntroStyled } from './styled';
...
export default function Intro({ groomName, brideName, locationName, date, message }: IntroProps) {
  return (
    <IntroStyled className={clsx('introStyled')}>
      <div className={clsx('wrap-persons')}>
        <span>{groomName}</span>
        <IconHeart />
        <span>{brideName}</span>
      </div>

      <Sections className="introLocation">
        <div className={clsx('wrap-location')}>
          <span>{format(parseISO(date), 'yyyy년 M월 d일 eeee', { locale: ko })}</span>
          <span>{locationName}</span>
        </div>
      </Sections>

      <IconFlower className={clsx('ico-flower')} />
...
function IconHeart() {
  return (
    <IconHeartSvgStyled width="512px" height="512px" version="1.1" viewBox="0 0 512 512">
